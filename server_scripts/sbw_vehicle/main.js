...
//
// 状态模型（3状态，保留 idle 条目便于排障）：
//   idle          — 空闲，无补员计划
//   timing        — 计时中，倒计时结束后将尝试部署
//   waiting_chunk — 等待区块加载，计时已结束但区块未加载
//
// 核心机制：
//   1. 定时循环检测（间隔 config.checkInterval tick）
//      遍历所有补员ID → 检查存活数 → 驱动状态机
//   2. 区块加载事件驱动（ChunkEvents.LOADED）
//      waiting_chunk 条目在目标区块加载时立即部署
//   3. 仅当"区块已加载 + 数量短缺"时执行 summon
//      避免区块未加载时的批量排队溢出
// ============================================================
 ...
 *   }
 * }
 * 注意：status="idle" 的条目也保留在 store 中，方便排障。
 */
...
function getChunkCoords(pos) { return { x: Math.floor(pos[0] / 16), z: Math.floor(pos[2] / 16) } }
...
function ensureVehicleEntry(server, vehicleId, vehicleCfg, teamName) {
  ...
  if (!store.vehicles[vehicleId]) {
    let chunk = getChunkCoords(vehicleCfg.pos)
 ...
 *   timing     → (数量达标)            → idle
 *   timing     → (计时到 + 区块已加载)  → idle (执行部署)
 *   timing     → (计时到 + 区块未加载)  → waiting_chunk
 *   waiting_chunk → (数量达标)         → idle
 *   waiting_chunk → (区块加载事件驱动)  → idle (由 ChunkEvents 触发)
 */
function checkReplenish(server, vehicleId, vehicleCfg) {
  ...
  if (aliveCount >= maxCount) {
    if (state.status !== 'idle') {
      ...
      state.status = 'idle'; state.timerStart = null; state.waitStart = null
      saveStore(server, store)
    }
    return
  }
  ...
  if (state.status === 'idle') {
    ...
    sbwLog('[补员] [' + vehicleId + '] 短缺 ' + (maxCount - aliveCount)
      + ' 辆，' + (delay / 20) + 's 后补员')
    saveStore(server, store)
    ...
    if (server.ticks - timerStart >= delay) {
      ...
      if (isChunkLoaded(server, vehicleCfg.pos[0], vehicleCfg.pos[2], dim)) {
        ...
        sbwLog('[补员] [' + vehicleId + '] 计时到但区块未加载，等区块')
        state.status = 'waiting_chunk'
        state.timerStart = null
        state.waitStart = server.ticks
        let c = getChunkCoords(vehicleCfg.pos)
        state.chunkX = c.x; state.chunkZ = c.z
      }
      saveStore(server, store)
    }

  } else if (state.status === 'waiting_chunk') {
    // 等待区块 → 什么都不做（由 ChunkEvents.LOADED 驱动）
  }
}

/**
 * 部署等待队列中的载具（由区块加载事件触发）
 * 部署前会二次确认数量是否仍然短缺
 */
function tryDeployWaitingVehicle(server, vehicleId, vehicleCfg) {
  let store = getStore(server)
  let state = store.vehicles[vehicleId]
  if (!state || state.status !== 'waiting_chunk') return false

  let aliveCount = countAliveByTag(server, getFullTag(vehicleId))
  if (aliveCount >= (vehicleCfg.maxCount || 1)) {
    state.status = 'idle'; state.waitStart = null
    saveStore(server, store)
    return false
  }

  spawnVehicleEntity(server, vehicleCfg)
  state.status = 'idle'; state.waitStart = null; state.uuid = null
  saveStore(server, store)
  return true
}
...
function deployVehicle(server, teamName, vehicleCfg) {
  ...
  if (!isChunkLoaded(server, vehicleCfg.pos[0], vehicleCfg.pos[2], getVehicleDimension(vehicleCfg))) {
    sbwLog('载具 [' + vehicleId + '] 区块未加载，无法立即部署')
    return
  }

  spawnVehicleEntity(server, vehicleCfg)
  let state = ensureVehicleEntry(server, vehicleId, vehicleCfg, teamName)
  state.status = 'idle'; state.uuid = null; state.timerStart = null; state.waitStart = null
...
function resetAll(server) {
  ...
  sbwLog('已重置：清除 ' + entityCount + ' 个载具实体')
    ...
    for (let i = 0; i < vehicles.length; i++) {
      ...
      if (entity) {
        ...
        if (state.status === 'timing') {
          ...
          lines.push(header + ' §7◐ 等区块 §7[' + state.chunkX + ', ' + state.chunkZ + ']')
        } else {
  ...
  for (let teamName in VEHICLE_CFG.teams) {
    ...
    for (let i = 0; i < vehicles.length; i++) {
      ...
      if (state.status === 'timing') {
        ...
        has = true
        lines.push('§7[' + teamName + '] §e' + v.id + ' §7— §7◐ 等区块 [' + state.chunkX + ', ' + state.chunkZ + ']')
      }
    }
  }
